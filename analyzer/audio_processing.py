...
# 1. Signal-Based Stress Scorer (Based on Research)
from tqdm import tqdm
import warnings
...
class AudioStressScorer:
    def __init__(self):
        # Psychoacoustic feature weights (research-derived)
        ...
        # Weights sum to 0.72 now, less extreme
        ...
        # Fit the scaler with some default values initially
        self.fit_with_defaults()
        
    def fit_with_defaults(self):
        # Create some dummy data to fit the scaler with reasonable defaults
        # This ensures the scaler is always fitted
    ...
    def extract_features(self, y, sr=44100):
        try:
            ...
            # Use updated librosa function for tempo
            try:
                # For librosa >= 0.10.0
            ...
            # Spectral features - ensure we get scalar values
            spectral_centroid = librosa.feature.spectral_centroid(y=y, sr=sr)[0]
            spectral_flatness = librosa.feature.spectral_flatness(y=y)[0]
            spectral_contrast = librosa.feature.spectral_contrast(y=y, sr=sr)
            
            # Take mean of spectral contrast across frequency bands
            ...
            # Return as flat array of scalar values
            return np.array([
                float(np.std(rms)),           # rms_variation
                float(tempo),                 # tempo
                float(np.mean(spectral_centroid)),  # spectral_centroid
                float(np.mean(spectral_flatness)),  # spectral_flatness
                float(spectral_contrast_mean),      # spectral_contrast
                float(onset_strength_mean),         # onset_strength
                float(harmonic_ratio),              # harmonic_ratio
                float(zero_crossing_mean),          # zero_crossing
                float(dissonance)                   # dissonance
            ])
    ...
    def _calculate_dissonance(self, y, sr, n_fft=2048):
        try:
            ...
            for i in range(1, min(100, len(freqs))):  # Limit to first 100 bins for efficiency
                for j in range(i+1, min(100, len(freqs))):
                    if freqs[i] > 0 and freqs[j] > 0:  # Avoid division by zero
                        freq_ratio = freqs[j] / freqs[i]
                        if 0.1 < freq_ratio < 1.2:  # Critical bandwidth region
    ...
    def fit(self, audio_files):
        ...
        if len(features) > 0:
            ...
            # Calculate mean and std for debugging
    ...
    def explain_stress_score(self, audio_path):
        """Explain which features contribute most to the stress score"""
        try:
            y, sr = librosa.load(audio_path, sr=44100, duration=30)
            features = self.extract_features(y, sr)
            scaled = self.scaler.transform(features.reshape(1, -1))[0]
            
            # Calculate contributions before sigmoid
            contributions = {}
